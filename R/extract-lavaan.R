...
# would carry a Wald interval and a p value on a posterior.
...
# know — including the capitalised `Component` values it prints —
# silently yields zero rows upstream (measured), so the argument is
# checked before easystats sees it. `"all"` wins over anything named
# with it.
check_sem_component <- function(component, call = rlang::caller_env()) {
  values <- c(
...
#' the model chi-square with its degrees of freedom and p value, CFI,
#' TLI, RMSEA with its confidence interval, and SRMR. The numbers come
#' from [lavaan::fitMeasures()]; apabayes computes no index of its own,
#' and none of its output judges a fit (ARCHITECTURE.md decision 24).
#'
...
#'   default is `0.90`.
#'
#' @return An [apabayes_tidy] tibble of type `"sem_fit"` with one row
#'   and columns `model`, `chisq`, `df`, `p`, `cfi`, `tli`, `rmsea`,
#'   `rmsea_low`, `rmsea_high`, `rmsea_level`, `srmr`; the blavaan
#'   columns (`ppp`, `brmsea`, `bgammahat` and their bounds) are `NA`.
#'   Attributes `estimator`, `test` and `n` record the estimator, the
#'   chi-square family reported and the sample size.
...
apa_tidy_sem_fit.lavaan <- function(x,
  ...
  # All measures at once rather than the named ones: asking `fitMeasures()`
  # for a name it does not know warns and drops it, and the check below
  # wants to name what is missing without that warning first.
  ...
  if (length(missing) > 0) {
    cli::cli_abort("{.arg x} reports no {.field {missing[1]}}.")
  }
